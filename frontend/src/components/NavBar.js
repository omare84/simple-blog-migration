    ...
    <nav className="bg-white shadow p-4">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        <div className="flex items-center space-x-6">
          <Link to="/" className="font-bold text-lg">ScalableDeploy</Link>
          <Link to="/features" className="text-sm hover:underline">Services</Link>
        ...
        <div className="flex items-center space-x-4">
          ...
          {user ? (
            <>
              <span className="text-sm text-gray-700 hidden sm:inline">Hi, {user.username}</span>
              <button
                onClick={signOut}
